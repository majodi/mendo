...
export class ArticlesBrwComponent extends BrwBaseClass<Article[]> implements OnInit, OnDestroy {
    ...
    {type: 'onValueChg', code: (ctrl, value) => {
      ...
      if (ctrl === 'measurements') {
        ...
        if (measurementsChoices) {
          const measurementsChoicesArray: Array<string> = measurementsChoices.split(',')
          ...
          for (const key in measurementsOverruleValue) {
      ...
      if (ctrl === 'colors') {
        ...
        if (colorsChoices) {
          const colorsChoicesArray: Array<string> = colorsChoices.split(',')
          ...
          for (const key in colorsOverruleValue) {
      ...
      if (fld === 'price' && !this.selectMode) {
        ...
        if (priceColDef !== undefined && priceColDef.headerSelectValue) {
          ...
          return this.ps.buttonDialog(dialogText, 'Bewaar', 'Annuleer', field).then(b => {
            if (b === 1) {
              const priceOverruleFld: any = rec['priceOverrule'] !== undefined ? rec['priceOverrule'] : {}
              if (field.value) {
  ...
  ngOnInit() {
    ...
    this.organisationSrv.initEntity$().pipe(takeUntil(this.ngUnsubscribe)).subscribe(organisations => {
