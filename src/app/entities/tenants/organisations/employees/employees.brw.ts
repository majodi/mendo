...
export class EmployeesBrwComponent extends BrwBaseClass<Employee[]> implements OnInit, OnDestroy {
  ...
  beforeForm(rec: Employee, fld) {
    ...
    if (fld === 'verificationCode') {
      this.db.getFirst('users', [{fld: 'email', operator: '==', value: rec.address.email}]).pipe(take(1)).toPromise().then((user: User) => {
        if (user && user['uid'] !== undefined) {
          const email = rec.address.email ? rec.address.email : '(Geen emailadres ingevuld)'
          const dialogText = user.employee !== undefined && user.employee === rec.id ?
          `Bestaand Account gevonden, koppeling met deze Medewerker reeds actief.` :
          `Bestaand Account gevonden, stuur verificatiecode om account te laten koppelen met deze medewerker.\r\n\r\nMedewerker:\r\n${rec.address.name}\r\n${email}\r\n\r\nVerificatiecode: ${rec.id}`
          this.ps.buttonDialog(
            dialogText, 'Sluit', (user.employee === undefined || user.employee !== rec.id) && rec.address.email ?
            'Stuur email' :
            undefined, undefined, user.employee === undefined || user.employee !== rec.id ?
            rec.id :
            undefined
          ).then(b => {
            if (b === 2) {
              const link = this.gs.tenantId !== undefined && rec.id !== undefined ?
              `https://us-central1-mendo-app.cloudfunctions.net/verifyemployee?user=${user['uid']}&tenant=${this.gs.tenantId}&code=${rec.id}` :
              'Verificatiecode: ' + rec.id
              const usage = this.gs.tenantId !== undefined && rec.id !== undefined ?
              'Klik op onderstaande link om uw account te verifiëren:' :
              'Voer onderstaande code in bij "Verificatie" in uw profiel (klik rechtsboven in de blauwe balk en kies Profiel):'
...
${this.gs.tenantName}
  ...
  allowedPropertiesForm(rec: Employee) {
    this.propertySrv.initEntity$().pipe(take(1), takeUntil(this.ngUnsubscribe), ).subscribe(properties => {
      const formValues = {propertiesAllowed: rec.propertiesAllowed}
      ...
      properties.forEach((property: Property) => {
        this.APFormConfig.push(
          {
            type: 'checkbox',
            label: 'Subselectie voor: ' + property.code,
            name: 'propertiesAllowed.' + property.id,
            placeholder: 'Subselectie voor: ' + property.code,
            value: rec.propertiesAllowed ? rec.propertiesAllowed[property.id] : false
          }
        )
        this.APFormConfig.push(
          {
            ...
            value: rec.propertiesAllowed && rec.propertiesAllowed[property.id] ?
              rec.propertiesAllowed[property.id].split(',').reduce((result, item) => {result[item] = true; return result}, {}) :
              {},
            options: property.choices.split(',').map(i => i.trim()),
            hidden: rec.propertiesAllowed ? !rec.propertiesAllowed[property.id] : true
          }
        )
      })
