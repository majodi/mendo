...
export class PopupService {
    ...
    buttonDialog(text, button1, button2?, field?, copyToClipboard?, htmlContent?, url?) {
      const dialogRef = this.dialog.open(PopupDialog, {
        width: '650px', // '325px',
        data: { text: text, but1: button1, but2: button2, field: field, copyToClipboard: copyToClipboard, htmlContent: htmlContent, url: url}
      })
    ...
    BrowseDialog(brwComponent: Type<any>, selectMode?: boolean, soberMode?: boolean, query?: QueryItem[], itemSelect?: boolean, itemSelectParent?: string, itemSelectEntity?: string) {
      ...
      const dialogRef = this.dialog.open(brwComponent, {
        width: '1000px',
        data: {}
      })
